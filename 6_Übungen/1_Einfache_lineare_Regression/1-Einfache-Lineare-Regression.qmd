...
```{r, eval = T, echo = F}
#| label: Evaluation Ausfleichsgeradenparameter
# Datenanalyse
fname       = "1-Einfache-Lineare-Regression.csv"                               # Dateiname
D           = read.table(file.path(fname), sep = ",", header = TRUE)            # Laden des Datensatzes
...
beta_hat_p_alt = matrix(rep(NaN, np * 2), nrow = 2)                             # Patient:innensteigungsparameter alternative Lösung
...
for(i in 1:np){                                                                 # Iteration über Patient:innen
  ...
  beta_hat_p_alt[2, i] = cxy/s2x                                                # \hat{\beta}_1, Steigungsparameter
  beta_hat_p_alt[1, i] = y_bar - beta_hat_p_alt[1, i] * x_bar                   # \hat{\beta}_0, Offset Parameter
  }

cat("       Mittelwert der Steigungsparameter         :", mean(beta_hat_p[2,]),
    "\n       Standardabweichung der Steigungsparameter :", sd(beta_hat_p[2,])
    # "\n       Mittelwert der Steigungsparameter (Theorem)        :", mean(beta_hat_p_alt[2,]),
    # "\n       Standardabweichung der Steigungsparameter (Theorem):", sd(beta_hat_p_alt[2,])
    )
```
